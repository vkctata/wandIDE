...
#[tauri::command]
fn delete_agent(id: String, db: State<Db>) -> Result<(), String> {
    let id = id.trim();
    if id.is_empty() {
...
fn disconnect_provider(provider: String, db: State<Db>) -> Result<(), String> {
    ...
    if provider == "azure-devops" {
        let conn = db.0.lock().map_err(|e| e.to_string())?;
        conn.execute(
            "DELETE FROM provider_settings WHERE provider='azure-devops'",
            [],
        )
        .map_err(|e| e.to_string())?;
    }
...
async fn test_provider_connection(
    ...
    let response = match provider.as_str() {
        ...
        "linear" => client
            ...
            .json(&serde_json::json!({"query":"query Viewer { viewer { id name } }"}))
            .send()
            .await,
    ...
    if provider == "linear" {
        ...
        if payload.get("errors").and_then(|value| value.as_array()).is_some_and(|errors| !errors.is_empty()) {
            return Err("Linear returned a GraphQL error".into());
        }
    }
...
#[tauri::command]
async fn sync_linear(db: State<'_, Db>, app: AppHandle) -> Result<Vec<ProviderRepo>, String> {
    let token = provider_token("linear").await?;
    let response = provider_http_client()?
        .post("https://api.linear.app/graphql")
        .header("Content-Type", "application/json")
        .header("Authorization", token)
        .json(&serde_json::json!({"query":"query Teams { teams(first: 100) { nodes { id name key } } }"}))
        .send()
        .await
        .map_err(|e| e.to_string())?;
    if !response.status().is_success() {
    ...
    let payload: serde_json::Value = response.json().await.map_err(|e| e.to_string())?;
    if payload.get("errors").and_then(|value| value.as_array()).is_some_and(|errors| !errors.is_empty()) {
        return Err("Linear returned a GraphQL error while loading teams".into());
    }
    ...
    for team in teams {
        ...
        let key = team["key"].as_str().unwrap_or_default();
        let name = team["name"].as_str().unwrap_or_default();
